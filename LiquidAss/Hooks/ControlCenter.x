...
static CGFloat ccGlassRadiusForMaterial(UIView *mat) {
    ...
    if (ccIsInsideSlider(mat)) {
        // Keep nested slider fill materials untouched, but allow the slider's
        // own background MTMaterialView to participate in Liquid (Gl)ass.
        // On iOS 17 brightness/volume use CCUIContinuousSliderView; MRU uses
        // the same direct-child pattern for media sliders.
        if (isExactClass(mat.superview, @"CCUIContinuousSliderView") ||
            isExactClass(mat.superview, @"MRUContinuousSliderView")) {
